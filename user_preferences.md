...
- **Tuyệt đối không Gán cứng Dữ liệu (No Hard-coding):** Giao diện (Frontend) không được phép gán cứng (hard-code) dữ liệu giả (mock data) trên view. Mọi thông tin, chỉ số và danh sách hiển thị phải được liên kết và lấy trực tiếp (fetch/bind) từ Backend thông qua API.
